# 06 — Service & Prompt Hooks (Automating the Rules)

Everything in `01`–`05` is discipline: you *remember* to do it each session.
...
## 6.1 The hook map — which hook does what
...
| C3 constraints upfront | remember declarations | `system.transform`: append the static part of your preamble (`02`, `05.2`) permanently; dynamic constraints still come from you |
...
## 6.3 Telemetry hooks — feed your own stats
...
## 6.4 Guardrail hooks (permission as policy)
...
## 6.5 Keep rules alive across compaction
...
```jsonc
{
  ...
  "instructions": ["AGENTS.md", "ai-user-training/00-quick-reference.md"]
}
```
...
## 6.6 When hooks are the wrong answer
