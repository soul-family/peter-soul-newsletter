...
Database schema: .dev-scripts/ai-sessions/shared/database-schema.jsonc
...
def find_sessions_by_directory(db_path, project_root):
    """Find all sessions in source DB whose directory matches project_root."""
    import sqlite3
    ...
    project_root_normalized = os.path.normpath(project_root).lower()

    c.execute('SELECT id, directory FROM session')
    matching_sessions = []

    for sid, directory in c.fetchall():
        if not directory:
            continue
        # Normalize directory for comparison
        dir_normalized = os.path.normpath(directory).lower()
        # Check if directory matches project root or contains it
        if dir_normalized == project_root_normalized or dir_normalized.startswith(project_root_normalized):
            matching_sessions.append(sid)
...
def backup_sessions(db_path, output_path, session_ids, path_replacements, developer):
...
def append_sessions_to_db(db_path, output_path, session_ids, path_replacements, developer):
...
def main():
    ...
    if args.sessions:
        ...
        print(f"Using {len(session_ids)} session IDs from command line")
    else:
        # First, try to load from existing backup database
        output_db = None
        if args.output_dir:
        ...
        if args.output_name:
            ...
            dev_config = get_developer_config(SCRIPT_DIR, args.developer)
            output_db = os.path.join(output_dir, dev_config.get('database', 'sessions.db'))
        ...
        if os.path.exists(output_db):
            try:
                ...
                conn.close()
                print(f"Loaded {len(session_ids)} session IDs from existing backup database")
            except Exception as e:
                print(f"Warning: Could not load from backup database: {e}")
        
        # If no backup DB or empty, load from config
        if not session_ids:
            session_ids = load_session_ids(SCRIPT_DIR, args.developer)
            print(f"Loaded {len(session_ids)} session IDs from config")
    
    # Expand: add sessions from source DB whose directory matches project root
    directory_matched = find_sessions_by_directory(args.db_path, project_root)
    if directory_matched:
        print(f"Found {len(directory_matched)} sessions in source DB matching project directory")
        for sid in directory_matched:
    ...
    if args.dry_run:
        ...
        count, msg_count, part_count = append_sessions_to_db(args.db_path, output_db, found, path_replacements, args.developer)
    else:
        print("\nCreating new database")
        count, msg_count, part_count = backup_sessions(args.db_path, output_db, found, path_replacements, args.developer)
