...
- Tasks use future-proof language (no file names, function names, or paths)
"""
...
from shared.future_proof import check_future_proof
...
def validate():
    ...
    for line in next_lines:
        ...
        if m:
            task = m.group(1)
            # Extract T-number if present
            t_match = re.match(r'(T-\d+):', task)
            task_id = t_match.group(1) if t_match else 'Unknown'
            
            if task in next_seen:
                issues.append(f"Duplicate task in todo-next.md: {task}")
            next_seen[task] = True
            
            # Check future-proof language
            issues.extend(check_future_proof(task, task_id))
