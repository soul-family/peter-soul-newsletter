...
How to use Kilo skills for the petersoul.co.uk archive project — benefits, calling syntax, inputs, and workflow guidance.
...
**Inputs to give:**
- `--db-path`: Path to the Kilo SQLite database (kilo.db)
- `--developer`: AI co-developer name — `kilo-code`, `opencode`, or `github-copilot` (default: `kilo-code`)
- `--current-session`: Flag to auto-discover and include the current/most-recent session ID
- `--website-sessions`: Comma-separated website session IDs (overrides JSON config)
- `--famtree-sessions`: Comma-separated famtree session IDs (overrides JSON config)
- `--paths-to-replace`: Comma-separated local paths to replace with `_www_` (optional)
  - `--output-dir`: Output directory (optional, defaults to `.ai-activity/ai-sessions/<developer>`)

**What it produces:**
- `website-sessions.db` and/or `famtree-sessions.db` in `.ai-activity/ai-sessions/<developer>/`
- Matching `.stats.json` files with per-session counts, timestamps, and totals
...
**Inputs to give:**
- Session transcript file from `_ai_session/`
- Session ID, title, and basic metadata
- The `ai-stats-template.md` template for metrics structure

**What it produces:**
- Analysis document in `.ai-activity/ai-analysis/`
- Recommendations in `.ai-activity/ai-outcomes/`
...
See also: [kilo-dev-guide.md](../dev-guides/kilo-dev-guide.md) | [ai-transparency-guide.md](ai-transparency-guide.md)
